...
PERSISTENCE = ".todo_list.json"

SERVICE_ITEM_SCHEMA = vol.Schema({vol.Required(ATTR_NAME): vol.Any(None, cv.string)})
SERVICE_LIST_SCHEMA = vol.Schema({})

WS_TYPE_TODO_LIST_ITEMS = "todo_list/items"
WS_TYPE_TODO_LIST_ADD_ITEM = "todo_list/items/add"
WS_TYPE_TODO_LIST_UPDATE_ITEM = "todo_list/items/update"
WS_TYPE_TODO_LIST_CLEAR_ITEMS = "todo_list/items/clear"

SCHEMA_WEBSOCKET_ITEMS = websocket_api.BASE_COMMAND_MESSAGE_SCHEMA.extend(
    {vol.Required("type"): WS_TYPE_TODO_LIST_ITEMS}
)

SCHEMA_WEBSOCKET_ADD_ITEM = websocket_api.BASE_COMMAND_MESSAGE_SCHEMA.extend(
    {vol.Required("type"): WS_TYPE_TODO_LIST_ADD_ITEM, vol.Required("name"): str}
)

SCHEMA_WEBSOCKET_UPDATE_ITEM = websocket_api.BASE_COMMAND_MESSAGE_SCHEMA.extend(
    {
        vol.Required("type"): WS_TYPE_TODO_LIST_UPDATE_ITEM,
...
SCHEMA_WEBSOCKET_CLEAR_ITEMS = websocket_api.BASE_COMMAND_MESSAGE_SCHEMA.extend(
    {vol.Required("type"): WS_TYPE_TODO_LIST_CLEAR_ITEMS}
)
...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    """Set up todo list from config flow."""
    ...
    data = hass.data[DOMAIN] = TodoData(hass)
    ...
    hass.http.register_view(TodoListView)
    hass.http.register_view(CreateTodoListItemView)
    hass.http.register_view(UpdateTodoListItemView)
    hass.http.register_view(ClearCompletedItemsView)

    frontend.async_register_built_in_panel(
        hass, "todo-list", "todo_list", "mdi:cart"
    )

    websocket_api.async_register_command(
        hass,
        WS_TYPE_TODO_LIST_ITEMS,
        websocket_handle_items,
        SCHEMA_WEBSOCKET_ITEMS,
    )
    websocket_api.async_register_command(
        hass,
        WS_TYPE_TODO_LIST_ADD_ITEM,
        websocket_handle_add,
        SCHEMA_WEBSOCKET_ADD_ITEM,
    )
    websocket_api.async_register_command(
        hass,
        WS_TYPE_TODO_LIST_UPDATE_ITEM,
        websocket_handle_update,
        SCHEMA_WEBSOCKET_UPDATE_ITEM,
    )
    websocket_api.async_register_command(
        hass,
        WS_TYPE_TODO_LIST_CLEAR_ITEMS,
    ...
    def async_reorder(self, item_ids):
        ...
        for key in all_items_mapping:
            ...
            if all_items_mapping[key]["complete"] is False:
                raise vol.Invalid(
                    "The item ids array doesn't contain all the unchecked todo list items."
                )
...
class TodoListView(http.HomeAssistantView):
    """View to retrieve todo list content."""
    ...
    def get(self, request):
        """Retrieve todo list items."""
        return self.json(request.app["hass"].data[DOMAIN].items)


class UpdateTodoListItemView(http.HomeAssistantView):
    """View to retrieve todo list content."""

    url = "/api/todo_list/item/{item_id}"
    name = "api:todo_list:item:id"
...
class CreateTodoListItemView(http.HomeAssistantView):
    """View to retrieve todo list content."""

    url = "/api/todo_list/item"
    name = "api:todo_list:item"
...
class ClearCompletedItemsView(http.HomeAssistantView):
    ...
    url = "/api/todo_list/clear_completed"
    name = "api:todo_list:clear_completed"
...
async def websocket_handle_clear(hass, connection, msg):
    """Handle clearing todo_list items."""
    await hass.data[DOMAIN].async_clear_completed()
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "todo_list/items/reorder",
        vol.Required("item_ids"): [str],
    }
)
def websocket_handle_reorder(hass, connection, msg):
    """Handle reordering todo_list items."""
    msg_id = msg.pop("id")
    try:
